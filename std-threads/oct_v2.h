...
namespace shared
{
	namespace v2
	{
		H_D_I void set_child(const int node_idx, int (*u_children)[8],
		                     int* u_child_node_mask, const unsigned int which_child,
		                     const int oct_idx)
		{
		...
		H_D_I void set_leaf(const int node_idx, int (*u_children)[8],
		                    int* u_child_leaf_mask, const unsigned int which_child,
		                    const int leaf_idx)
		{
		...
		H_D_I void process_oct_node(const int i /*brt node index*/,
		                            // --------------------------
		                            int (*u_children)[8], glm::vec4* u_corner,
		                            float* u_cell_size, int* u_child_node_mask,
		                            // --------------------------
		                            const int* node_offsets,
		                            // prefix sum
		                            const int* rt_node_counts,
		                            // edge count
		                            const unsigned int* codes, const uint8_t* rt_prefixN,
		                            const int* rt_parents, const float min_coord,
		                            const float range)
		{
			...
			for (auto j = 0; j < n_new_nodes - 1; ++j)
			{
				...
				set_child(parent, u_children, u_child_node_mask, which_child, oct_idx);

				// compute the corner of the current octnode
				morton32_to_xyz(&u_corner[oct_idx],
				                node_prefix << (morton_bits - (3 * level)), min_coord,
				                range);
			...
			if (n_new_nodes > 0)
			{
				...
				set_child(oct_parent, u_children, u_child_node_mask, which_child, oct_idx);
		...
		H_D_I void process_link_leaf(const int i /*brt node index*/,
		                             // --------------------------
		                             int (*u_children)[8], int* u_child_leaf_mask,
		                             // --------------------------
		                             const int* node_offsets, const int* rt_node_counts,
		                             const unsigned int* codes,
		                             const bool* rt_hasLeafLeft,
		                             const bool* rt_hasLeafRight,
		                             const uint8_t* rt_prefixN, const int* rt_parents,
		                             const int* rt_leftChild)
		{
			if (rt_hasLeafLeft[i])
			{
				...
				set_leaf(bottom_oct_idx, u_children, u_child_leaf_mask, child_idx, leaf_idx);
			}
